...
- TS + `<script setup>`；分号、单引号、`@antfu/eslint-config`；Naive UI + Tailwind（少用 `<style>`）；文案同步 `locales/zh-CN.ts` 与 `en.ts`
...
`lint` 必跑；IPC/UI/外部命令尽量 `dev`；打包改动验 `package`/`make`。勿提交密钥/私路径/产物；未确认勿 push git、发博客、跑 Hexo；小步、贴合现有结构。
